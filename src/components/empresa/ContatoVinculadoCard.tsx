...
export function ContatoVinculadoCard({ pessoa, onClick }: Props) {
  ...
  return (
    ...
    >
      <div className="w-8 h-8 rounded-full bg-accent/20 flex items-center justify-center flex-shrink-0">
        <span className="text-accent font-semibold text-[13px]">{initial}</span>
      </div>
      <div className="flex-1 min-w-0">
        ...
        {pessoa.cargo && (
          <p className="text-[11px] text-slate-500 truncate">{CARGO_LABELS[pessoa.cargo]}</p>
        )}
      </div>
      {pessoa.telefone && (
        <button
          ...
          onClick={(e) => {
            e.stopPropagation()
            window.open(`https://wa.me/55${stripMask(pessoa.telefone!)}`, '_blank')
          }}
          title="WhatsApp"
          className="text-green-500 hover:text-green-400 opacity-0 group-hover:opacity-100 transition-all flex-shrink-0"
        >
          {WA_SVG}
        </button>
      )}
