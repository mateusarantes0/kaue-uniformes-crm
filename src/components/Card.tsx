...
import { Orcamento, ORIGEM_LABELS, TIPO_OBJECAO_LABELS, COLUNA_BADGE, COLUNA_LABELS } from '../types'
      ...
      {(provided, snapshot) => (
        ...
        >
          ...
          <div className="flex items-start justify-between gap-1 mb-1.5">
            <div className="min-w-0 flex-1">
              ...
              <button
                ...
                className="text-white font-semibold text-sm leading-tight text-left hover:text-accent transition-colors truncate block w-full"
              >
                {o.nome}
              </button>
              {empresa && (
            ...
            <div className="flex items-center gap-0.5 flex-shrink-0">
              {primeiroContato?.telefone && (
                <button
                  onClick={() => window.open(`https://wa.me/55${stripMask(primeiroContato.telefone!)}`, '_blank')}
                  title="WhatsApp"
              ...
              <button onClick={() => setModalEditar(o)} title="Editar" className="text-slate-400 hover:text-white p-1 rounded transition-colors">
          ...
          <div className="space-y-1">
            {contatos.length > 0 && (
              <p className="text-slate-500 text-xs">👥 {contatos.length} contato{contatos.length > 1 ? 's' : ''}</p>
            )}
            ...
            {o.origem && (
              <p className="text-slate-500 text-xs">{ORIGEM_LABELS[o.origem]}</p>
            )}
