...
function corBorda(orc: Orcamento): string {
  const ref = orc.ultimoContatoEm ?? orc.criadoEm
  const dias = Math.floor((Date.now() - new Date(ref).getTime()) / 86400000)
  if (dias > 7) return 'border-l-red-500'
  if (dias <= 1) return 'border-l-emerald-500'
...
export function Card({ orcamento: o, index }: CardProps) {
  ...
  const outrosContatos = contatos.length - 1
    ...
    <Draggable draggableId={o.id} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={`group rounded-lg p-3 mb-2 cursor-pointer active:cursor-grabbing transition-shadow select-none bg-card border-l-4 ${corBorda(o)} ${snapshot.isDragging ? 'shadow-2xl ring-1 ring-primary-light/50' : 'hover:shadow-lg'}`}
        >
          {/* Top row: ID + badges + WhatsApp */}
          <div className="flex items-start justify-between gap-1 mb-1.5">
            <div className="flex items-center gap-1.5 min-w-0 flex-1 flex-wrap">
              <span className="text-[10px] text-slate-500 font-mono flex-shrink-0">{o.id}</span>
              {itensPendentes > 0 && (
                <span className="text-[10px] bg-amber-900/40 text-amber-400 px-1.5 py-0.5 rounded-full leading-none flex-shrink-0">
                  {itensPendentes}✓
                </span>
              )}
              {atividadeBadge && (
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full leading-none font-medium flex-shrink-0 ${
                  atividadeBadge === 'hoje'
            ...
            {primeiroContato?.telefone && (
              <button
                ...
                title="WhatsApp"
                className="text-green-500/70 hover:text-green-400 p-0.5 rounded transition-colors flex-shrink-0"
              >
          ...
          {/* Title — protagonist */}
          <p className="text-[15px] font-semibold text-white leading-tight truncate mb-0.5">{o.nome}</p>
            ...
            <p className="text-[12px] text-slate-400 truncate mb-1.5">
              {primeiroContato ? (
                <>
                  ...
                  {outrosContatos > 0 && <span className="text-slate-500"> +{outrosContatos}</span>}
          ...
          {o.valor != null && o.valor > 0 && (
            <p className="text-[13px] mb-1.5">
              <span className="text-accent font-mono font-semibold">{formatCurrency(o.valor)}</span>
              {o.probabilidade != null && (
                <span className="text-slate-400"> · {o.probabilidade}%</span>
              )}
            </p>
          )}
          ...
          <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-700/50">
            <span className={`text-[11px] font-medium ${dias > 7 ? 'text-red-400' : 'text-slate-500'}`}>
